...
void lv_textarea_add_char(lv_obj_t * obj, uint32_t c)
{
    ...
    if(ta->pwd_mode) {
        /*+2: the new char + \0*/
        size_t need_size = lv_strlen(ta->pwd_tmp) + lv_strlen(letter_buf) + 1;
...
void lv_textarea_add_text(lv_obj_t * obj, const char * txt)
{
    ...
    if(ta->pwd_mode) {
        size_t need_size = lv_strlen(ta->pwd_tmp) + lv_strlen(txt) + 1;
        if (need_size > ta->pwd_tmp_len) return;
...
void lv_textarea_delete_char(lv_obj_t * obj)
{
    ...
    if(ta->pwd_mode) {
        size_t new_size, orig_size = lv_strlen(ta->pwd_tmp);
        lv_text_cut(ta->pwd_tmp, ta->cursor.pos - 1, 1);
        new_size = lv_strlen(ta->pwd_tmp);
        lv_memzero(ta->pwd_tmp + orig_size, orig_size - new_size);
    }
...
void lv_textarea_set_text(lv_obj_t * obj, const char * txt)
{
    ...
    if(lv_textarea_get_accepted_chars(obj) || lv_textarea_get_max_length(obj)) {
        ...
        if(ta->pwd_mode) {
            lv_memzero(ta->pwd_tmp, ta->pwd_tmp_len);
        }
        uint32_t i = 0;
        while(txt[i] != '\0') {
            uint32_t c = lv_text_encoded_next(txt, &i);
    ...
    if(ta->pwd_mode) {
        if (lv_strlen(txt) + 1 > ta->pwd_tmp_len) return;
        lv_memzero(ta->pwd_tmp, ta->pwd_tmp_len);
...
void lv_textarea_set_max_length(lv_obj_t * obj, uint32_t num)
{
    ...
    if (ta->pwd_mode) {
        ...
        lv_memcpy(new_alloc, ta->pwd_tmp, ta->pwd_tmp_len);
#if LV_MEM_CUSTOM_SECURE == 1
        LV_MEM_CUSTOM_SECURE_FREE(ta->pwd_tmp, ta->pwd_tmp_len);
#else
        lv_memzero(ta->pwd_tmp, ta->pwd_tmp_len);
...
static void pwd_char_hider(lv_obj_t * obj)
{
    ...
    lv_memzero(txt_tmp, enc_len * bullet_len + 1);
    lv_free(txt_tmp);
